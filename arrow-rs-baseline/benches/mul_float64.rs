...
fn bench_mul_float64(c: &mut Criterion<OpsMsWallTime>) {
    let mut group = c.benchmark_group("mul_float64");
    ...
    for rows in ROWS {
        ...
        group.bench_function(format!("rows_{rows}"), |b| {
            ...
            b.iter(|| {
                ...
                let out = mul(left_ref, right_ref).expect("arrow mul float64 should succeed");
