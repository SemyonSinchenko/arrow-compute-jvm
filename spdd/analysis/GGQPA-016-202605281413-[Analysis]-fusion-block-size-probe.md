# SPDD Analysis: Block-Size Probe for Runtime-Codegen Fusion (MulFloat64 Chain)
...
- Fusion block strategy: already codified in `CORE_DESIGN.md` and `BENCHMARKS.md` as the project’s selected fusion direction and benchmark question framing.
- MulFloat64 baseline kernel family: `MulFloat64Raw`, `MulFloat64` wrapper, `MulDispatch`, and `Compute.mul(...)` define the canonical multiply semantics and public API path used as probe baselines.
- Runtime codegen probe substrate: `MulFloat64CodeGen` + `JaninoLoader` already establish Janino compile/load mechanics and method-handle invocation shape at k=1.
- Benchmark comparison model: `MulFloat64DispatchBenchmark`, `MulFloat64CodeGenBenchmark`, and `BenchmarkMetadataProvider` provide the established rows grid, metadata protocol, and reused vs per-call allocation comparison conventions.
- Cross-runtime native reference lane: `arrow-rs-baseline` Cargo project and existing `mul_float64.rs` define the non-JVM interpreted-vector baseline pattern for external comparison.

#### New Concepts Required
- Fusion block size envelope: explicit business concept of lower/upper usable k bounds (fusion floor and fusion ceiling) that future optimizer logic will consume.
- Chained single-input multiply workload: formal benchmark workload (`x^k`, left-associative) used as controlled probe shape for scaling behavior.
- AOT fused anchor family: handwritten chain-depth-specific kernels as “upper bound” control for isolating codegen-vs-fusion effects.
- Janino chain codegen artifact: parameterized per-k dynamic class generation contract (fresh class per k, no cross-k bytecode reuse).
- Inlining diagnostics deliverable: structural performance-explainability artifact (C2/inlining/bytecode-size evidence), treated as a required report output.

#### Key Business Rules
- Left-associative equivalence rule: all execution strategies must compute the same association order to keep comparisons semantically valid.
- Steady-state policy rule: compile latency and cold-start behavior remain outside this probe’s measured objective.
- Fairness-by-shape rule: A1/A2 must use public `Compute.mul(...)`; B/C must use reused outputs; D must be null-free to avoid validity-buffer confounding.
- Probe-only governance rule: this SPDD defines design evidence and documentation stance; implementation artifacts ship in downstream iteration, not as acceptance for this requirement doc.
...
#### Solution Direction
- Use a controlled benchmark-probe architecture that extends existing SPDD 15 codegen infrastructure and benchmark governance, then derive two actionable block-size thresholds (floor/ceiling) rather than selecting a block size a priori.
- Preserve current project layering: public API chain baseline (`Compute`/dispatch/wrapper), isolated codegen path (Janino + method handle), and external arrow-rs lane; compare them under one shared workload axis (k).

#### Key Design Decisions
- Block fusion (k-sized units) vs whole-pipeline fusion: lower structural complexity and better fit for warmed JVM engines, at cost of explicit inter-block materialization boundaries → recommend block fusion as default strategic stance (already aligned with `CORE_DESIGN.md`).
- Per-k dynamic class isolation vs shared/generic generated class: improves observability of method-size/inlining breakpoints and maps directly to the “huge method” risk question, at cost of more compile artifacts → recommend per-k isolation for this probe phase.
- Upper-bound anchor (AOT fused) inclusion vs Janino-vs-naive only: stronger causal attribution for scaling limits, at cost of extra implementation surface → recommend keeping AOT anchor mandatory.
- Reporting inlining diagnostics as first-class output vs throughput-only reporting: improves explainability and future optimizer confidence, at cost of additional PR analysis work → recommend mandatory diagnostics reporting.

#### Alternatives Considered
- Whole-pipeline fusion benchmark first: rejected because it introduces optimizer/IR concerns before establishing empirically safe block-size bounds.
- Throughput-only benchmark without JIT diagnostics: rejected because breakpoint interpretation would remain ambiguous (HotSpot limit vs other noise).
- Generic multi-operator chain in first probe: rejected because it adds shape variance and weakens attribution to block-size scaling itself.
...
- Break-even threshold operationalization: ±15% “notebook noise envelope” is stated, but the exact statistical decision rule is not explicitly fixed (single run vs median across forks).
- Inlining diagnostics source-of-truth: multiple acceptable tooling paths are allowed (PrintInlining, JFR, jcmd), but expected normalization format is unspecified.
- Ping-pong parity convention wording: requirement allows either parity convention if documented; inconsistent convention across implementations could distort A1 results.

#### Edge Cases
- Large-k method growth edge: generated source/bytecode may approach practical JIT heuristics before hard bytecode limit; failure mode may be non-linear at specific rows.
- Floating-point special-value propagation across deep chains: NaN/Inf/signed-zero behavior is deterministic but can mask subtle association drift if parity fixtures are incomplete.
- Very small row sizes: per-call overhead can dominate and invert expected ranking, affecting interpretation of fusion floor if only largest-row conclusions are emphasized.

#### Technical Risks
- HotSpot inlining cliff risk: Janino-fused method may stop full C2 inlining at higher k, reducing fused advantage; mitigation direction is diagnostic-backed block-size bounding.
- Benchmark comparability risk across cells: allocation/lifecycle asymmetry can contaminate interpretation if output-policy discipline drifts; mitigation direction is strict cell-shape invariants and shared consumption pattern.
- Cross-runtime fairness risk (JVM vs arrow-rs): differences in runtime/tooling can be over-interpreted as architecture claims; mitigation direction is explicit positioning as structural probe, not language-speed claim.
- Scope creep risk into optimizer design: requirement is intentionally pre-optimizer; mitigation direction is enforce probe-only deliverables and defer “how” to downstream REASONS/implementation phases.

#### Acceptance Criteria Coverage
| AC# | Description | Addressable? | Gaps/Notes |
|-----|-------------|--------------|------------|
| 1 | Requirement file exists with required structure | Yes | Already satisfied by `spdd_requirements/requirements/16-fusion-block-size-probe.md`. |
| 2 | `CORE_DESIGN.md §Expression fusion` amended with block-fusion/SPDD16 stance | Yes | Present in current `CORE_DESIGN.md` section; traceability wording exists. |
| 3 | `BENCHMARKS.md §Fusion benchmarks` forward-reference to SPDD16 block-size question | Yes | Present in “Fusion benchmarks” question subsection. |
| 4 | `BENCHMARKS.md §Phase 5: Fusion` concrete SPDD16 reference replacing provisional language | Yes | Present with A1/A2/B/C + arrow-rs framing. |
| 5 | No implementation code/build changes required for this SPDD acceptance | Yes | Current repository state aligns with doc-only acceptance posture; implementation deferred downstream. |
