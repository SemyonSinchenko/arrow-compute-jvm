# SPDD Analysis: Arrow-rs Peer Positioning and Caller-Owned Buffers
...
- Compute facade and explicit dispatch: `Compute` keeps ergonomic `FieldVector` API while `AddDispatch`/`DivideDispatch` route to concrete wrappers — business purpose is a stable external surface with typed execution internally.
- Fast-tier wrapper layer: classes like `AddInt32`, `DivInt32`, `SumInt64`, and `StartsWithUtf8` own boundary validation, validity handling, and raw-kernel invocation — relationship is wrapper as contract boundary above Arrow-free raw kernels.
- Memory-lifetime utilities: `BufferRefs`, `SegmentViews`, `Checks`, `Validity` are established shared primitives for retain/release, memory bridging, preconditions, and validity propagation.
- Benchmark governance layer: dispatch benchmarks currently include `wrapperEval` and pre-retain setup; benchmark suite expresses wrapper cost as a first-class measured concept.
- Foundation docs as normative architecture: `CORE_DESIGN.md`, `AGENTS.md`, `ARROW_JAVA_API_USAGE.md`, and `spdd_requirements/README.md` already codify invariants and are treated as source of truth.

#### New Concepts Required
- Caller-owned-buffer contract as primary safety model: lifetime responsibility shifts from wrapper internals to caller obligations, aligning JVM layer semantics with arrow-rs/arrow-cpp expectations.
- Thin wrapper concept: wrappers remain for validation/dispatch semantics but must stop structural overhead behaviors (per-call retain cycle, eager all-valid materialization, repeated segment extraction).
- Monomorphic wrapper-entry invariant: concrete vector signatures in wrappers become a locked architectural rule rather than an incidental current state.
- Documentation-first contract alignment: public Javadoc and foundation docs become explicit deliverables of the contract, not optional commentary.
- Transitional safe-mode separation: heavy/safety wrapper behavior is treated as deferred optional mode, conceptually isolated from MVP fast-tier path.

#### Key Business Rules
- Wrapper lifetime rule: wrappers do not retain buffers per call; callers must keep all vectors live for call duration — governs `Compute`, dispatch, wrapper methods, and memory helper semantics.
- Null-validity rule: on `null_count == 0`, wrappers do not materialize validity bits and must rely on null-count convention; on nullable paths, validity propagation still applies — governs wrapper and validity helper behavior.
- Performance-integrity rule: per-call allocations in checks/lifetime guardrails are disallowed in hot wrapper paths — governs `Checks`/`BufferRefs` usage model.
- Architectural truthfulness rule: docs, benchmark framing, and API contracts must all describe the same audience and contract, with no residual “casual-user safety boundary” framing.
- Scope boundary rule: this SPDD changes wrapper-contract/documentation direction and benchmark coverage expectations, but not raw kernel logic or broad architecture beyond named amendments.
...
#### Solution Direction
- Adopt a contract-led refactor direction where wrapper behavior, benchmark interpretation, and foundation documentation are aligned around “arrow-rs peer” positioning.
- Preserve existing layering (`Compute` facade -> dispatch -> concrete wrapper -> raw kernel) while changing responsibility boundaries inside that layering.
- Treat this requirement as a governance-and-architecture convergence effort: documentation and invariant updates land as first-class outputs before downstream code iteration.

#### Key Design Decisions
- Lifetime ownership boundary: wrapper-managed retain/release vs caller-owned lifetime -> caller-owned eliminates structural fence/allocation overhead and matches target audience mental model; recommended direction is caller-owned contract with `BufferRefs` retained only as optional public utility.
- Validity handling strategy: eager all-valid bitmap writes vs lazy Arrow null-count semantics -> eager writes simplify consumers but add avoidable bandwidth cost; recommended direction is lazy validity on all-valid path with explicit null-count observability.
- API polymorphism location: polymorphic wrappers vs polymorphic facade + monomorphic wrappers -> keeping polymorphism at facade preserves usability while protecting hot path specialization; recommended direction is lock monomorphic wrapper entries as invariant.
- Segment-view lifecycle model: repeated ad hoc extraction per call vs amortized extraction at wrapper boundary -> repeated extraction risks object churn and unstable EA outcomes; recommended direction is one-extraction-per-call-path (or equivalent cache) with no segment escape.
- Contract delivery sequence: code-first vs docs-and-javadoc-first -> code-first risks transient contradiction with foundation truth; recommended direction is docs/javadoc contract declaration before implementation changes.

#### Alternatives Considered
- Keep current safe-wrapper contract and optimize micro-paths only: rejected because measured overhead sources are structural (ownership semantics and eager operations), not isolated micro-inefficiencies.
- Move directly to a new parallel `Compute.Safe.*` API in MVP: rejected because requirement defers safe-mode as v2 and prioritizes thin-wrapper default path.
- Remove `BufferRefs` entirely: rejected because caller workflows (cross-stage/thread retention) still need an explicit utility even after wrapper decoupling.
...
- Wrapper coverage boundary: requirement says “every fast-tier `<Op><Type>.eval(...)`” gets contract javadoc, but current package naming includes `safe`, `validonly`, and `agg`; exact inclusion boundary should be verified for consistency.
- Thin-wrapper benchmark naming transition: requirement allows `wrapperEvalThin` addition or repurposing `wrapperEval`; reporting policy for historical comparability is not fully fixed.
- Segment amortization mechanism: two options are allowed (cache field vs per-entry extraction), but no explicit criterion is given for choosing one under different vector types.

#### Edge Cases
- Empty-row batches: wrappers still call `setValueCount`; with lazy validity, consumers that incorrectly inspect validity bits despite `null_count == 0` may surface latent downstream assumptions.
- Slow-tier consistency: requirement says slow-tier inherits caller-owned contract but internal shape remains untouched; documentation mismatch risk exists if only fast-tier docs are updated.
- Mixed-nullity operations: valid-only kernels like `DivInt32` have precheck logic tied to propagated active validity; behavior must remain deterministic when all rows inactive.
- Slice-offset enforcement: requirement keeps zero-slice checks while targeting zero allocation; preserving correctness without varargs across unary/binary/ternary entry points is a subtle boundary.

#### Technical Risks
- Leak-detection migration risk: moving retain responsibility outward may shift failures from wrapper scope to caller batch-close points, increasing debugging distance -> mitigation direction: strengthen caller-facing docs/examples and keep allocator-debug invariant active in tests.
- Contract drift risk: code may evolve while foundation docs stay stale, or vice versa -> mitigation direction: atomic change-set discipline across `CORE_DESIGN.md`, `AGENTS.md`, `ARROW_JAVA_API_USAGE.md`, and `spdd_requirements/README.md`.
- Performance-claim risk: removing known overhead may not produce uniform parity across all row sizes and null profiles -> mitigation direction: require benchmark matrix updates and stated measurement margin before headline claim change.
- ABI/surface confusion risk: keeping `FieldVector` facade while tightening wrapper signatures can be misread as unchanged semantics -> mitigation direction: explicit facade-vs-wrapper responsibility wording in Javadoc and design docs.

#### Acceptance Criteria Coverage
| AC# | Description | Addressable? | Gaps/Notes |
|-----|-------------|--------------|------------|
| 1 | `13-arrow-rs-peer-positioning.md` exists with required structure | Yes | Already present and complete in repository. |
| 2 | `CORE_DESIGN.md` multi-section amendments (public API, wrapper layer, memory model, segment lifetime wording, `BufferRefs` demotion, `Validity` note, design decisions, risk target) | Partial | File already contains several caller-owned updates; full line-by-line conformance to all listed subsection edits still needs deliberate verification in downstream implementation pass. |
| 3 | `AGENTS.md` amendments (caller responsibility framing, lifetime dependency wording, API phrasing, default invariant bullet) | Yes | Current `AGENTS.md` includes caller-owned lifetime framing and default-invariant bullet for caller-owned buffers. |
| 4 | `ARROW_JAVA_API_USAGE.md` §2 and §15 caller-owned contract updates | Yes | Current file explicitly states wrappers do not retain and slow-tier follows same contract. |
| 5 | `spdd_requirements/README.md` core constraints lines 49–51 updated | Yes | Current constraints include caller-owned lifetime, no wrapper retain, and no segment escape. |
| 6 | `11-bench-cleanup-and-cargo-reference.md` forward-reference retarget + framing-footnote amendment | No | File still points to `13-onebrc-...` and footnote still frames wrapper tax as fusion-only without SPDD 13 refinement. |
| 7 | `12-native-baseline.md` superseded notice unchanged | Partial | Not directly analyzed in this scoped pass; should be verified unchanged during implementation iteration. |
| 8 | No Java source changes included in SPDD acceptance | No | Current codebase still contains heavy-wrapper behavior (`BufferRefs.retain`, `Validity.markAllValid`), which is expected pre-implementation but means requirement-state and code-state are intentionally out of sync pending downstream iteration. |
