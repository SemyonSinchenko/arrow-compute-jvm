...
final class MulFloat64Test {
    ...
    private void assertProfile(int n, double leftNullRate, double rightNullRate) {
        try (var root = new RootAllocator();
             var child = root.newChildAllocator("mul-float64-wrapper", 0, Long.MAX_VALUE);
            ...
            for (int i = 0; i < n; i++) {
                ...
                double rv = i % 11 == 0 ? Double.NEGATIVE_INFINITY : 100.0d - i * 0.25d;
                leftValid[i] = lValid;
                rightValid[i] = rValid;
                leftVals[i] = lv;
                rightVals[i] = rv;
