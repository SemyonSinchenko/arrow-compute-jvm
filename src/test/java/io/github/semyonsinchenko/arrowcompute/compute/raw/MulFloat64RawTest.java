...
final class MulFloat64RawTest {
    ...
    void computeAll_isBitExactAgainstScalarReference() {
        int species = MulFloat64Raw.SPECIES.length();
        assertCase(1);
        assertCase(species);
        assertCase(species + 7);
    }
    ...
    private void assertCase(int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            double rv = switch (i % 5) {
                ...
                case 3 -> -2.0d;
                default -> i * 0.75d + 1.0d;
            };
            left.set(MulFloat64Raw.FLOAT64_LE, off, lv);
            right.set(MulFloat64Raw.FLOAT64_LE, off, rv);
        }

        MulFloat64Raw.computeAll(left, right, out, n);

        for (int i = 0; i < n; i++) {
            long off = (long) i * Double.BYTES;
            double expected = left.get(MulFloat64Raw.FLOAT64_LE, off) * right.get(MulFloat64Raw.FLOAT64_LE, off);
            long expectedBits = Double.doubleToRawLongBits(expected);
            long actualBits = Double.doubleToRawLongBits(out.get(MulFloat64Raw.FLOAT64_LE, off));
